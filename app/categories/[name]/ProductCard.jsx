...
export default function Card({ id, name, image, price, catName }) {
    ...
    useEffect(() => {
        if (eyeClicked) document.body.style.overflow = "hidden";
    ...
    const handleWishlist = () => {
        ...
        addToWishlist(product); // Add product to wishlist context
    };

    const handleCart = () => {
        setClicked(!clicked);   // optional: toggle button color
    ...
    return (
        <>
            <div className="group relative flex flex-col hover:shadow-[0_0_12px_rgba(32,181,38,0.32)] border rounded-[8px] border-[#E6E6E6] bg-[#FFFFFF]">
                <img src={image} alt={name} className="w-full h-auto sm:h-[280px] md:h-[300px] rounded-t-[8px]" />
                ...
                <div className="absolute top-5 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition">
                    ...
                    <div
                        ...
                        className={`w-[32px] h-[32px] sm:w-[38px] sm:h-[38px] rounded-full flex items-center justify-center cursor-pointer transition-all duration-200 border border-[#F2F2F2] ${heartClicked ? "bg-red-500" : "bg-white"} hover:bg-red-400 hover:shadow-md`}>
                    ...
                    <div
                        ...
                        className={`w-[32px] h-[32px] sm:w-[38px] sm:h-[38px] rounded-full flex items-center justify-center cursor-pointer transition-all duration-200 border border-[#F2F2F2] ${eyeClicked ? "bg-[#00B207]" : "bg-white"} hover:bg-[#00cc27] hover:shadow-md`}>
                ...
                <div className="flex items-center p-4 justify-between">
                    ...
                    <div
                        onClick={handleCart}
                        className={`w-[30px] sm:w-[40px] h-[30px] sm:h-[40px] flex items-center justify-center rounded-full cursor-pointer transition ${clicked ? "bg-[#00B207]" : "bg-[#F2F2F2]"}`}>
