...
export default function Products() {
    ...
    return (
        <section className="ml-5 mt-6 mt-15 mb-15 mr-5 sm:ml-9 sm:mr-9">
            ...
            <div className="grid gap-6 lg:grid-cols-5 sm:grid-cols-2 grid-cols-1 gap-3">
                ...
                {prod.map((item) => (
                    <ProductsCard key={item.id} id={item.id} price={item.price} name={item.name} image={item.image} />
                ))}
            </div>
        </section>
    );
